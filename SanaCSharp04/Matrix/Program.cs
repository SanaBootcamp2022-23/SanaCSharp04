...
Console.WriteLine($"\n1) Кількість додатних елементів в матриці: {PositiveNumbersCount}");
...
if (ValuesRepeatedMoreThan2Times.Count > 0)
    Console.WriteLine($"\n2) Максимальне число яке повторюється більше 1 разу в матриці : {ValuesRepeatedMoreThan2Times.Max()}");
else
    Console.WriteLine($"\n2) Максимального числа яке повторюється більше 1 разу в матриці немає.");
...
RowsNoZeroCount = Matrix.GetLength(0) - RowsNoZeroCount;

Console.WriteLine($"\n3) Кількість рядків матриці які не містять жодного нульового елемента: {RowsNoZeroCount}");
...
Console.WriteLine($"\n4) Кількість стовпців матриці які містять хоча б один нульовий елемент: {RowsZeroCount}");
