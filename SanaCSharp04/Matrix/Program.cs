...
if(ValuesRepeatedMoreThan2Times.Count > 0)
    Console.WriteLine($"\n2) Максимальне число яке повторюється > 1 разу в матриці {N}x{M}: {ValuesRepeatedMoreThan2Times.Max()}");
else
    Console.WriteLine($"\n2) Максимального числа яке повторюється > 1 разу в матриці {N}x{M} немає.");
