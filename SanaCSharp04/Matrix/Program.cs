...
for (int i = 0; i < N; i++)
{
    for (int j = 0; j < M; j++)
    {
        ...
        if (matrix[i, j] > 0) positiveCount++; //відповідь на завдання "кількість додатних елементів"
    }
    Console.WriteLine();
}
...
for (int i = 0; i < N; i++)
{
    for (int j = 0; j < M; j++)
    {
        ...
        for (int i1 = 0; i1 < N; i1++)
        {
            ...
            if (repeat > 1 && current > repeatedCount) repeatedCount = current;
        }
    }
}
Console.WriteLine($"Максимальне із чисел, що зустрічається в заданій матриці більше одного разу = {repeatedCount}"); //максимальне із чисел, що зустрічається в заданій матриці більше одного разу
...
for (int i = 0; i < N; i++)
{
    ...
    for (int j = 0; j < M; j++)
    {
        ...
        if (matrix[i, j] == 0) haveZeroCols++;
    }
    if (rowsPos == N)
    {
        for (int j1 = 0; j1 < N; j1++)
        {
            dobutokCountHaventZeroRows *= matrix[i, j1]; 
        }
    }
    if (rowsZero == N) haventZeroRows++;
}
...
for (int k = 1 - N; k < M; k++)
{
    ...
    if (sum > maxSumParalelDiagonal) maxSumParalelDiagonal = sum;
}
...
for (int j = 0; j < N; j++)
{
    ...
    if (cols == M)
    {
        ...
        {
            sumCountHaventZeroCols += matrix[i1, j]; 
        }
    }
}
...
for (int j = 0; j < M; j++)
{
    ...
    for (int i1 = 0; i1 < N; i1++)
    {
        if (negEl > 0) sumElNegCols += matrix[i1, j]; 
}
